                    ...
                    <div class="col-md-12">
                        ...
                        <div class="content-panel">
                            <Div class="centered">
                                ...
                                <br>
                                <h4>DETAILS OF Previous Loan STATEMENT</h4>
                                @endforeach
                                @endif
                            </Div>
                            <br>
                                    ...
                                    <div class="content-panel">
                                        <div class="adv-table" style="overflow-x:auto;">
                                            <table cellpadding="0" cellspacing="0" border="0"
                                                ...
                                                <tbody>
                                                    @if(!empty($prev_loan_data))
                                                    @foreach ($prev_loan_data as $data)
                                                    <tr>
                                                        <td>{{++$sr}}</td>
                                                        <td>{{$data->loan_no}}</td>
                                                        <td>{{$data->amount}}</td>
                                                        <td>{{$data->interest}}</td>
                                                        <td>{{$data->amount + $data->interest + $data->agent_commission - + $data->shares_amount}}</td>
                                                        <td>{{$data->loan_start_date}}</td>
                                                        <td>{{$data->interest_calculated_up_to}}</td>
                                                        <td>
                                                            <select name="select_action" class="form-control col-sm-6"
                                                                id="select_action"
                                                                onchange="location = this.options[this.selectedIndex].value;">
                                                                <option selected disabled>Select Action</option>
                                                                <option value="{{url('/loan_statement/'.$data->id)}}">
                                                                    Loan Statement
                                                                </option>
                                                                <option
                                                                    value="{{url('/monthly_loan_statement/'.$data->id)}}">
                                                                    Monthly Status
                                                                </option>
                                                            </select>
                                                        </td>
                                                        <td><a onclick="return myconfirmation()"
                                                                href="{{url('/remove_loan/'.$data->id)}}"
                                                                class="btn btn-danger btn-md fa fa-trash-o">
                                                            </a></td>
                                                    </tr>
                                                    @endforeach
                                                    @endif
